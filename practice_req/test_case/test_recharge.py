...
logger=logg.logger('recharge')
read = do_excel.doExcel(route.exc_1)
cases_2 = read.get_data('recharge')

@ddt
class TestRecharge(unittest.TestCase):                               #测试登录
    @classmethod
    ...
    #     url_1 = str(get_api+item.case_url)
        ...
        try:
            ...
            logger.info('执行通过')
            read.write_data('recharge',item.case_id+1,result,'pass')
        except AssertionError as e:
            logger.error('执行出错：{0}'.format(e))
            read.write_data('recharge',item.case_id + 1, result, 'fall')
            raise e
    @classmethod
