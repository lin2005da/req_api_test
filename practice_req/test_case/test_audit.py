...
logger=logg.logger('audit')
read = do_excel.doExcel(route.exc_1)
cases_2 = read.get_data('audit')


@ddt
class Testaudit(unittest.TestCase):                               #测试登录
    @classmethod
    ...
    #     url_1 = str(get_api+item.case_url)
        item.case_data=context.str_re(item.case_data)
        ...
        try:
            ...
            logger.info('执行通过')
            read.write_data('audit',item.case_id+1,result,'pass')
        except AssertionError as e:
            logger.error('执行出错：{0}'.format(e))
            read.write_data('audit',item.case_id + 1, result, 'fall')
            raise e
    @classmethod
